    ...
    public function initializeColumnsOptimized(array $columns): array
    {
        ...
        foreach ($columns as $index => $column) {
            if (is_string($column)) {
                ...
                && !isset($column['relation'])
            ) {
                // Raw-only static display column: DO NOT assign a database 'key'
                // Use a stable identifier derived from label/raw for visibility and rendering only
                $derived = $column['label'] ?? $column['raw'] ?? "column_{$index}";
                // Sanitize identifier (but keep it separate from DB key on purpose)
                $identifier = preg_replace('/[^a-zA-Z0-9_]/', '_', (string) $derived);
                $identifier = trim($identifier, '_');
                if ($identifier === '' || is_numeric($identifier)) {
                    $identifier = "column_{$index}";
                }

                $optimizedColumns[$identifier] = array_merge([
                    // Intentionally no 'key' here so it will never be selected from DB
                    'label' => $column['label'] ?? ucfirst(str_replace('_', ' ', $identifier)),
                ...
                $optimizedColumns[$key] = array_merge([
                    // In fallback, this may not represent a real DB column. Avoid assuming it is.
                    // Only set a 'key' if caller explicitly provided one in $column (handled above).
                    'label' => ucfirst(str_replace('_', ' ', $key)),
                    'sortable' => false,
                    'searchable' => false,
